...
describe('LoginUserUseCase', () => {
  ...
  it('1. Supabase local user login thành công', async () => {
    usersService.findByEmail.mockResolvedValue({ id: 'u1', supabaseId: 'sb_id', isActive: true, role: Role.CUSTOMER });
    supabaseAuthService.signInEmail.mockResolvedValue({ user: { id: 'sb_id', email_confirmed_at: '2023-01-01' } });
  ...
  it('2. Supabase ID mismatch -> exact conflict behavior', async () => {
    usersService.findByEmail.mockResolvedValue({ id: 'u1', supabaseId: 'sb_id_old', isActive: true });
    supabaseAuthService.signInEmail.mockResolvedValue({ user: { id: 'sb_id_new' } });
    
    await expect(useCase.execute({ email: 'test@example.com', password: 'pass' })).rejects.toThrow(ConflictException);
  });
  ...
  it('3. Email chưa verified -> exact forbidden behavior', async () => {
    ...
    supabaseAuthService.signInEmail.mockResolvedValue({ user: { id: 'sb_id', email_confirmed_at: null } });
    
    await expect(useCase.execute({ email: 'test@example.com', password: 'pass' })).rejects.toThrow(ForbiddenException);
  });

  it('4. Legacy bcrypt user login thành công và không gọi Supabase', async () => {
    usersService.findByEmail.mockResolvedValue({ id: 'u1', supabaseId: null, passwordHash: 'hash', isActive: true });
  ...
  it('5. Legacy bcrypt wrong password -> exact unauthorized behavior', async () => {
    ...
    await expect(useCase.execute({ email: 'test@example.com', password: 'pass' })).rejects.toThrow(UnauthorizedException);
  });

  it('6. Local account incomplete -> exact conflict behavior', async () => {
    usersService.findByEmail.mockResolvedValue({ id: 'u1', supabaseId: null, passwordHash: null, isActive: true });
    await expect(useCase.execute({ email: 'test@example.com', password: 'pass' })).rejects.toThrow(ConflictException);
  });
  ...
  it('8. Inactive user -> exact locked behavior', async () => {
    usersService.findByEmail.mockResolvedValue({ id: 'u1', supabaseId: 'sb_id', isActive: false, emailVerifiedAt: new Date() });
    supabaseAuthService.signInEmail.mockResolvedValue({ user: { id: 'sb_id', email_confirmed_at: '2023-01-01' } });
